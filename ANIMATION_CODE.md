# Welcome Screen Animation - Complete Code

This document contains all the code needed to recreate the elegant welcome screen animation with typing effects, shimmer animations, and spring-animated checkmarks.

## Overview

This animation features:
- Soft, breathing gradient background that moves slowly
- ChatGPT-style letter-by-letter typing animation
- Subtle shimmer effect using palette colors (#6c574c, #d0baaf, #e1d2ca)
- Circular radio indicators that morph into checkmarks after exactly 2 seconds
- Spring animations for smooth, magical transitions
...
## File: svg-8fo5b06xca.ts

Create this file to store the SVG path data:
...
import svgPaths from './svg-8fo5b06xca';
    ...
    <div className="-translate-x-1/2 absolute bottom-0 contents left-[calc(50%+25.81px)] top-0" data-name="Battery">
      ...
      <div className="-translate-x-1/2 absolute bottom-[31.87%] left-[calc(50%+38.81px)] top-[36.78%] w-[1.328px]" data-name="Cap">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 1.32804 4.45283">
          <path d={svgPaths.pb2786c0} fill="var(--fill-0, black)" id="Cap" opacity="0.4" />
        </svg>
      </div>
    ...
    <div className="absolute h-[14.204px] left-[21.35%] right-[22.9%] top-[22.4px]">
      ...
      <div className="-translate-x-1/2 absolute bottom-[0.26%] left-[calc(50%-3.58px)] top-[4.91%] w-[17.142px]" data-name="Wifi">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 17.1417 13.4698">
          <path clipRule="evenodd" d={svgPaths.p323dee00} fill="var(--fill-0, black)" fillRule="evenodd" id="Wifi" />
        </svg>
      </div>
      <div className="-translate-x-1/2 absolute bottom-[1.83%] left-[calc(50%-29.25px)] top-[4.12%] w-[19.2px]" data-name="Cellular Connection">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 19.2 13.3585">
          <path clipRule="evenodd" d={svgPaths.p1c1a8200} fill="var(--fill-0, black)" fillRule="evenodd" id="Cellular Connection" />
        </svg>
      </div>
    </div>
  );
...
function Time() {
  return (
    <div className="-translate-y-1/2 absolute h-[59px] left-0 right-[64.25%] top-1/2" data-name="Time">
      <div className="absolute flex flex-col font-['SF_Pro:Semibold',sans-serif] font-[590] inset-[0_36.65%_0_37.01%] justify-center leading-[0] text-[17px] text-black text-center" style={{ fontVariationSettings: "'wdth' 100" }}>
...
function StatusBar() {
  return (
    <div className="-translate-x-1/2 absolute bg-[rgba(255,255,255,0)] h-[59px] left-1/2 overflow-clip top-0 w-[430px]" data-name="Status Bar">
      <Levels />
      <Time />
    </div>
  );
...
function AnimatedItem({
  ...
  // ~30ms per character for human-like typing (slightly faster than real typing)
  ...
  useEffect(() => {
    if (isTypingComplete && startTime) {
      // Calculate remaining time to reach 2 seconds total
    ...
    <AnimatePresence>
      {isVisible && (
        <>
          ...
          {/* Typed text */}
          <div
            className="absolute font-['SF_Pro_Display:Semibold',sans-serif] font-normal leading-[26px] text-[20px] w-[331px] whitespace-pre-wrap"
            style={{ top, left }}
          >
            <div className="relative">
              {/* Base text (shows after shimmer stops) */}
              <p
                className={gradient ? 'bg-clip-text bg-gradient-to-r from-[#6c574c] to-[#d2a994] to-[66.559%]' : 'text-[#6c574c]'}
                style={{
...
export default function App() {
  ...
  useEffect(() => {
    // Start the first item after a brief delay
  ...
  useEffect(() => {
    if (currentStep >= 0 && currentStep < items.length - 1) {
      // Calculate time needed: typing (30ms * length) + shimmer completion (2000ms total) + small buffer (300ms)
      const currentText = items[currentStep].text;
      const typingTime = currentText.length * 30;
      const totalTime = Math.max(2000, typingTime) + 500; // At least 2s, plus 500ms buffer
...
### 1. **Typing Animation**
- Each sentence types out character by character at 30ms per character
- Creates a natural, human-like typing rhythm

### 2. **Shimmer Effect**
- Uses a gradient that moves across the text: `linear-gradient(90deg, ...)`
- Colors are from the palette with reduced opacity
- Animates `backgroundPosition` from `0%` to `200%` over 2 seconds
- Repeats infinitely until the checkmark appears

### 3. **2-Second Timing**
- Tracks `startTime` when typing begins
- After typing completes, calculates remaining time to hit exactly 2 seconds total
- Shows checkmark after that remaining time

### 4. **Spring Animations**
- Circle indicator: `stiffness: 400, damping: 25, mass: 0.8`
- Checkmark: `stiffness: 500, damping: 18, mass: 0.5`
- Creates a bouncy, satisfying effect

### 5. **Breathing Background**
- Multiple gradient layers animate at different speeds (8s, 20s, 25s, 30s)
- Creates subtle light movement across the screen
- Opacity and position changes create a "breathing" quality

## Customization

### Change Typing Speed
In the `AnimatedItem` component, adjust this value:
```typescript
}, 30); // Change this number (lower = faster)
```

### Adjust Shimmer Duration
In the shimmer animation:
```typescript
transition={{
...
### Update Colors
Change the shimmer gradient colors:
```typescript
backgroundImage:
  'linear-gradient(90deg, rgba(108, 87, 76, 0.5) 0%, ...)',
```

## Notes

- The animation is designed for a 430px wide mobile viewport
- Font used: SF Pro Display (system font on iOS)
- Tailwind CSS is used for styling
- Motion (Framer Motion) handles all animations

---

**Created:** February 3, 2026  
**Framework:** React + TypeScript + Tailwind CSS + Motion  
**Animation Type:** Loading sequence with typing effects
